...
- Our **judge** = an LLM-as-a-Judge that emits a structured `Analysis`
  ...
  the premise that *convergence is not correctness*.
...
1. **Multi-agent debate for LLM-as-judge.** Instead of running `n` judges once
   ...
   *iteratively refine* their judgments over rounds; debate terminates early on
...
2. **A Bayesian "latent concept" model of debate.** Each task has a hidden
   ...
   *provided at least one agent starts from the correct concept*.
...
4. **Empirical tuning.** Ensemble size of **7** is the sweet spot (5 is often
   enough; ≥9 shows diminishing returns). Debate helps most on *hard,
   high-variance* tasks; on easy/high-consensus tasks plain majority vote is
   comparable or better.

### What can be used in this project

- **Adaptive / conditional escalation.** Their headline practical insight —
  *spend extra compute only where it changes the answer* — is directly
...
- **Bimodal-consensus intuition for routing.** Their finding that debate
  ...
  treating *disagreement* as the signal that warrants the expensive judge +
...
- This is an **LLM-as-judge** paper, and we have an explicit judge stage. It
  validates moving *beyond majority voting*, which is exactly what our reasoning
  synthesizer does (it is an authoritative integrator, not a vote counter).
- The theory's key precondition — *debate only helps if at least one agent seeds
  the correct concept* — reinforces our **panel-diversity** recommendation in
...
- **Caution:** full iterative debate adds rounds (latency + tokens) and
  ...
  deliberately preserves. Adopt the *adaptive-compute* idea first; treat
...
1. **CARA (Cross-Agent Reasoning Alignment).** A family of *post-hoc* metrics
   (no extra LLM calls) that ask: among agents that agree on the *answer*, do
   they agree on the *reasoning*? Each rationale is split into steps; pairwise
...
2. **The "consistency illusion" finding.** Standard multi-agent debate *reduces
   visible contradictions (CR ↓) while also reducing reasoning similarity
   (SIM ↓)* — agents look like they agree more but actually reason less
   ...
   convergence** (an agent adopts the majority answer with *zero* reasoning).
   Debate even *manufactures* disagreement: the "no-majority" rate jumped ~38×
...
3. **Grounded Debate Protocol (GDP).** A pure **prompt-level** intervention (no
   ...
   - **Ground** — a *named* fact, mechanism, or guideline supporting the claim.
   ...
   *eliminated* both severe failure modes, while leaving answer accuracy roughly
...
- **"Claim + Ground" grounding in our judge and synthesis prompts.** This is the
  ...
  `trigger` and `evidence` per issue; GDP generalizes that discipline to *every*
  ...
  - Have the **judge** record, per agreement, *why each agreeing model believed
    it* — turning our `agreements` array from "they converged" into "they
...
- **Anti-sycophancy clause.** A near-verbatim port of GDP's rule belongs in our
  ...
  prompt: *do not adopt a position merely because others hold it.*
...
- It is **empirical proof of the exact assumption our prompts are built on.**
  `synthesis-prompt.ts` already says *"Convergence does not imply correctness"*
  and *"candidates trained on similar data share the same blind spots."* This
  paper measures that effect and shows naive debate makes it *worse*, not
  better. It strongly justifies our design choice that the synthesizer is an
  authority that *independently verifies* convergent claims rather than trusting
  agreement.
...
- The illusion is *larger on open-ended tasks* with more room for reasoning to
...
- **Authors / venue:** Roumeliotis, Margaris, Spiliotopoulos, Vassilakis — *Future Internet* 2026, 18(2):112
...
1. **Reasoning meta-model aggregation vs. statistical aggregation.** 12 base
   ...
   **reasoning LLM "meta-model" (GPT-5 / GPT-5-mini)** then receives *both the
   base predictions and the original input text* and produces the final answer
...
4. **Override / divergence analysis.** The meta-model matched the majority ~86%
   of the time, but *was more accurate when it diverged* (77.7% vs 70.4%), and
   ...
   (disagreement = an implicit low-confidence signal). Decision *quality* mattered
   more than override *frequency* (GPT-5 mini overrode more but did worse).
...
- **Lightweight synthesizer is viable.** GPT-5-mini nearly matched GPT-5 as the
  meta-model — *the aggregation architecture mattered more than the meta-model's
  raw strength.* Reassuring for users who run a modest synthesizer, though the
...
- The **self-preference bias** they observed is a real risk for us: the README
  ...
  This paper quantifies *why* — the aggregator over-trusts its own family. Good
  ammunition for keeping the judge independent from the panel.
- Their **consensus→accuracy** curve says agreement *is* informative on
  easy/clear inputs — a useful counterbalance to Paper 2. The synthesis logic to
  aim for: *trust strong consensus by default, but escalate skepticism as
  agreement weakens or reasoning fails to align.*
- **Scope caveat:** this is bounded, ordinal (1–5 star) classification on
  ...
  stronger). Treat the *direction* of these findings as transferable, the exact
  numbers as not.
...
1. **Parallel Synthesis architecture.** Three cognitively specialized roles run
   **in parallel and in isolation** (no peer visibility), then a Synthesizer
   ...
   - **Synthesizer** — distributed-cognition integrator (Hutchins): find
     convergence (treat as a reliability signal), resolve divergence *by
     evidence not majority*, build a unified answer preserving the reasoning
     chain.
...
3. **Emergent Reasoning Score (ERS).** A composite metric splitting:
   ...
   - **Emergent Value (EV):** fraction of synthesized concepts that were in *no*
...
4. **Key finding.** SE was high (~0.71–0.74) but EV was low (~0.10): the
   ...
   Majority voting matched or slightly beat the synthesizer *on answer accuracy*,
...
- **Reframe / extend our `thinkingMode`s as cognitive roles.** We already inject
  ...
  complementary, *non-overlapping* role set we could offer as additional modes
...
- **Our `synthesis-prompt.ts` already matches their Synthesizer recipe.** Their
  ...
  response preserving the chain) is nearly identical to our instructions 3–4 and
  10. This is strong cross-validation; their explicit wording
  ("when two roles reach the same conclusion via *different* paths, treat that
...
- **ERS as an offline evaluation harness.** SE/EV could be implemented as a test
  or eval script for *our* synthesizer: does the final answer actually
...
- **Set expectations honestly.** Their result — integration ≫ emergence, and
  ...
  magically produces *new* correctness. Our honest pitch should be: reliable,
  multi-perspective, *verified* integration (and error-correction via the judge),
...
- **Cognitive-function > persona** is a useful principle for how we word
  `thinking-modes.ts`: target the reasoning *process*, not a job title (our
  current modes already do this — keep it that way).
- **Diminishing returns when one role is weak.** They found a weak role (Critic
  on MMLU) *dragged synthesis down* toward a diluted compromise. For us this
...
1. **"Convergence ≠ correctness" is now well-supported (Papers 1, 2, 3, 4).**
   Our prompts already encode it; keep it and make it *quantitative* by surfacing
...
4. **Keep the reasoning synthesizer; it beats statistical aggregation (Paper
   3)** and reliably integrates perspectives (Paper 4) — but be honest that its
   value is *verified integration and error-correction*, not novel-answer
   generation.
