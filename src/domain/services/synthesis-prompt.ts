...
12. OUTPUT CONTRACT — If the original conversation specifies how the final answer must be formatted (for example: reply with a single letter or word, respond only with valid JSON of a stated shape, end with a line of the form 'Answer: X', or obey a word/character/length limit), your response MUST satisfy that format exactly, including any required literal tokens, prefixes, or markers. This contract takes precedence over the default synthesis verbosity and the "scale rigor" guidance: when the task demands a terse or fixed-format answer, output exactly that and nothing more — no preamble, commentary, or restating of the question. When the panel candidates already conform to the required format, preserve it rather than reformatting it into prose.`;
