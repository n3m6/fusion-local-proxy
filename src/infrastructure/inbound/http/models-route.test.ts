...
test('GET /v1/models returns panel model in data array', async () => {
  const config = stubConfigPort([makeModelRef('gpt-4o')], null, makeModelRef('synth'));
  const app = new Hono();
...
test('GET /v1/models includes judge model when configured', async () => {
...
test('GET /v1/models returns only synthesizer when no panel or judge configured', async () => {
  const config = stubConfigPort([], null, makeModelRef('synth-only'));
  const app = new Hono();
  app.get('/v1/models', createModelsRoute(config));
  ...
  assert.equal(data[0].id, 'synth-only');
  assert.equal(body.object, 'list');
});
