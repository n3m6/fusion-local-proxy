...
Extend the domain model with an expanded `PanelResult` interface (the current stub has only `modelId` and `content` — it must gain `provider`, `usage`, and `latencyMs`) and a new `PanelMeta` envelope. Create the `PanelRunner` application use case that dispatches parallel calls to N panel models via `Promise.allSettled`, collects both successful results and partial failures, and raises `FusionError('all_panels_failed')` only when every panel model fails.

### Current state of fusion-types.ts

The existing `PanelResult` interface in `src/domain/model/fusion-types.ts` currently defines only:
```typescript
...
```
This stub must be expanded to include `provider`, `usage`, and `latencyMs` as specified below. The `PanelMeta` interface does not exist and must be created.

### Domain type additions (`fusion-types.ts`)

Expand the existing `PanelResult` interface and append `PanelMeta`:

- **`PanelResult`** — expanded from the current stub. Fields:
  - `modelId: string` — the model identifier string (e.g. `"gpt-4o"`) — **existing, retained**
  - `content: string` — the raw text response from the model — **existing, retained**
  - `provider: ProviderType` — `'openai'` or `'anthropic'` — **new**
  - `usage: { promptTokens: number; completionTokens: number }` — token counts from the `ChatResponse.usage` map — **new**
  - `latencyMs: number` — wall-clock duration in milliseconds measured between dispatching the per-model call and receiving the response — **new**
...
Also document the `all_panels_failed` error code convention in a JSDoc comment on the `FusionError` class: thrown with code `'all_panels_failed'` when `Promise.allSettled` produces zero fulfilled results.
...
## Files
- `src/domain/model/fusion-types.ts` (MODIFY) — Expand existing `PanelResult` interface (currently `{ modelId, content }`) to include `provider: ProviderType`, `usage: { promptTokens: number; completionTokens: number }`, and `latencyMs: number`. Append `PanelMeta` interface (`results: PanelResult[]`, `failedModels: FailedModelInfo[]`). Import `FailedModelInfo` from `stream-types.js`. Document `all_panels_failed` error code convention on the `FusionError` class via JSDoc.
